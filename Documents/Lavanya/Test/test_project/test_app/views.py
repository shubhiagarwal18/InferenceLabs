...
def render_category_products(request, category_id):
    ...
    try:
        ...
        if products_by_category.status_code == 200:
            ...
            # reviews = ProductReview.objects.filter(product__id__in=product_ids)
            ...
            return render(request, 'products_by_category.html', {
                ...
                # 'reviews_by_product_id': reviews_by_product_id,
...
def submit_review(request, product_id):
    ...
    reviews_by_product_id = {review.product.id: [] for review in reviews}
    [reviews_by_product_id[review.product.id].append(review) for review in reviews]

    form = ProductReviewForm(request.POST) if request.method == 'POST' else ProductReviewForm()
